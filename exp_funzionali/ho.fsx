...
2) v e' il valore dell'espressione expr (espressione a destra di ->)  nell'ambiente 
   ...
   In altri termini, si calcola il valore di expr dove x e' sostituito con vt.
...
// non viene valutata la sottoespressione   (4/0 > 0)
...
Notare che Env non contiene un binding  per w.
Se Env non contenesse alcun binding per z, la valutazione solleverebbe una eccezione.
