...
(**  expv: ESPRESSIONI  INTERE  CONTENENTI VARIABILI     **)
...
let rec evalv e ( env : envt)  =
    ...
    | Diff(e1,e2)  -> evalv e1 env  - evalv e2 env
    | Prod(e1,e2)  -> evalv e1 env  * evalv e2 env ;;
...
// Towards typing: a type expressions containg integers and booleans
...
but an exp. We'll see why shortly
...
let rec tpchko e tenv  = 
    ...
    | B(_) -> Some BOOL
    | Sum(e1,e2) | Prod(e1,e2) | Diff(e1,e2) -> 
                    let (t1,t2) = (tpchko e1 tenv, tpchko e2 tenv)
                    if t1 = Some INT && t2 = Some INT 
                        then t1 
                            else None
                            // 
    | Eq(e1,e2)  -> let (t1,t2) = (tpchko e1 tenv, tpchko e2 tenv)
                    if t1 = t2 && not (t1 = None) 
                                   then Some BOOL
                                     else None
    | Less(e1,e2)  -> let (t1,t2) = (tpchko e1 tenv, tpchko e2 tenv)
                      if t1 = Some INT && t2 = Some INT 
                                   then Some BOOL
                                     else None
    | Neg b -> (tpchko b tenv);;
...
let main  e tenv env = 
    try
        let t = tpchk e tenv
        let v = eval e env
        printf "%A has type %A and value %A\n"  et t v
    with 
        | :? System.Exception -> printf "%A does not type check\n" et

let ok1 = main  et2 atenv anenv;; 
let notok1 = main et3 atenv anenv;; 

// awful, but recovers the error message
